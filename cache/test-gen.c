...
#define MAXN 256 // maximum array dimension
...
extern void registerFunctions(); // external function defined in trans.c
...
void eval_perf(unsigned int s, unsigned int E, unsigned int b) {
    ...
    for (int i = 0; i < func_counter; ++i) {
        ...
        if (results.funcid == i) // save the correctness of the transpose submission
            results.correct = 1;
        ...
        while (fgets(buf, 1000, full_trace_fp) != NULL) {
            if (buf[0] == ' ' && buf[2] == ' ' && (buf[1] == 'S' || buf[1] == 'M' || buf[1] == 'L')) {
                ...
                if (addr == marker_start) // set flag if start marker found
                    flag = 1;
                ...
                if (addr == marker_end) { // if end marker found, close trace file
                    flag = 0;
        ...
        if (results.funcid == i) // if it is transpose_submit(), record number of misses
...
int main(int argc, char *argv[]) {
    ...
    if (signal(SIGSEGV, sigsegv_handler) == SIG_ERR) { // install SIGSEGV and SIGALRM handlers */
    ...
    alarm(120); // time out and give up after a while
    eval_perf(5, 1, 5); // check the performance of the student's transpose function
